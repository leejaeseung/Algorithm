...
풀이:
MST(최소 신장 트리)를 약간 변형하면 됩니다.
크루스칼 알고리즘을 이용했습니다.
merge() 함수에서 정점들이 합쳐질 때마다 그룹의 카운트를 하나씩 낮춰줍니다.(그룹은 처음엔 N개 있습니다 - 아무것도 연결되지 않았을 때)
그러다 그룹이 1개가 되면 마을이 분리되지 않은 것이므로 가중치를 더하지 않고 종료합니다.(즉, 그룹이 2개일 때까지만 더합니다)
그러면 그리디하게 최소값을 구할 수 있습니다.
*/
#include<iostream>
#include<string>
#include<algorithm>
...
int main(void) {
	...
	for (int i = 0; i < M; i++)
	{
		...
		pq.push(make_pair(-v, make_pair(n1, n2)));
	}
