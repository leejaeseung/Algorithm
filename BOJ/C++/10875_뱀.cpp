...
좌표계의 크기가 너무 크므로, 뱀이 이동할 때마다 선분을 만들어 주어 해결합니다.

방향을 바꿔 이동할 때마다 이전에 만들어진 모든 선분들을 비교하여 두 선분이 겹치는지 확인합니다.

좌표계를 벗어나는 경우도 따로 체크해 줍니다.
단, 자신의 몸에 부딪히지 않고 모든 방향 변경이 종료되는 경우, 벽까지 계속 이동하므로 마지막 이동에
매우 큰 이동 한 번을 큐에 넣어주어 해결했습니다.
*/
#include<iostream>
#include<algorithm>
#include<math.h>
