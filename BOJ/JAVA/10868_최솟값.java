...
public class Main {
    ...
    public static void main(String[] argc) throws IOException {
        ...
        for (int i = 0; i <M ; i++) {
            ...
            bw.write(getMin(tree,1,1,N,Integer.parseInt(st.nextToken()),Integer.parseInt(st.nextToken()))+"\n");
        }
        bw.flush();
        bw.close();
    }
    public static long init(long[] tree,long[] num,int now,int start,int end){
        ...
        return tree[now]=Math.min(init(tree,num,now*2,start,(start+end)/2),init(tree,num,now*2+1,(start+end)/2+1,end));
    }
