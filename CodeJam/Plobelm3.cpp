...
bool can(vector<pii> &vt, int s, int e) {
	for (int i = 0; i < vt.size(); i++)
	{
		if ((vt[i].first < e && vt[i].first >= s) || (vt[i].second > s && vt[i].second <= e) || (vt[i].first <= s && vt[i].second >= e))
			return false;
	}
	return true;
}
...
int main(void) {
	...
	while (T--) {
		...
		for (int i = 0; i < N; i++)
		{
			int s, e;
			cin >> s >> e;
			pq.push(make_pair(make_pair(-s, -e), i));
		}
		...
		while (!pq.empty()) {
			...
			if (can(CV, s, e)) {
				CV.push_back(make_pair(s, e));
				ans[idx] = 'C';
			}
			else {
				if (can(JV, s, e)) {
					JV.push_back(make_pair(s, e));
					ans[idx] = 'J';
				}
				else
					imp = true;
