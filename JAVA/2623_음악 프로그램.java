...
출력은 N 개의 줄로 이뤄지며, 한 줄에 하나의 번호를 출력한 다. 이들은 남일이가 정한 가수들의 출연 순서를 나타낸다. 답이 여럿일 경우에는 아무거나 하나를 출력 한다. 만약 남일이가 순서를 정하는 것이 불가능할 경우에는 첫째 줄에 0을 출력한다.
    풀이 :
    위상 정렬에서 루프가 걸리는 상황을 check배열로 다 돌았는지 확인하여 처리한다.
 */
import java.io.*;
import java.util.*;
...
public class Main {
    ...
    public static void TPS(Node[] nd) throws IOException{
        Queue<Node> q=new LinkedList<>();
        boolean[] check=new boolean[nd.length];
